...
function checkEligibility() {
    ...
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(function(position) {
                ...
                if (isEligible && (Math.abs(current_loc.lat - petition["latitude"]) <= 0.0006) && (Math.abs(current_loc.lng - petition["longitude"]) <= 0.0006)) {
...
function filterPetiton(inHour, inLoc, inCallback) {
    ...
    playersRef.once("value").then(function(snapshot) {
        ...
        for (var o in petitions) {
            ...
            // if (true) {
            if ((Math.abs(inLoc.lat - lat) <= 0.0006) && (Math.abs(inLoc.lng - lng) <= 0.0006)) {
