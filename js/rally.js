...
function fetchSignature(inDateIndex, inDateRange, inBldgIdx, inCulumtative) {
    if (inDateIndex == inDateRange.length) {
        if (conn["cnt"] + slow["cnt"] > 0) {
            ...
            $("#number").text(["제보", (conn["cnt"] + slow["cnt"]), "개", "(저번 주 누적 제보", (inCulumtative), "개)"].join(" "));

            $("#stat").css("display", "block");
        } /* END. When exist report. */
        else {
            $("#number").text("이번 주에 아직 제보한 사람이 없습니다. 친구들에게 홍보해 더 많은 힘을 모아보세요! (저번 주 누적 제보 " + inCulumtative + "개)");
